...
@section('title', 'Chỉnh sửa khách hàng - Keki SaaS')
...
<div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600 mt-1">
                    Cập nhật thông tin khách hàng và hệ số cược
                </p>
            </div>
    ...
    <!-- Form -->
    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('user.customers.update', $customer) }}">
            @csrf
            @method('PUT')
            ...
            <!-- Customer Statistics -->
            <div class="mb-8">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Thống kê khách hàng</h3>
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="bg-gray-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">Tổng ăn</p>
                        <p class="text-lg font-semibold text-green-600">{{ number_format($customer->total_win_amount, 0, ',', '.') }} VNĐ</p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">Tổng thua</p>
                        <p class="text-lg font-semibold text-red-600">{{ number_format($customer->total_lose_amount, 0, ',', '.') }} VNĐ</p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">Lãi/Lỗ tổng</p>
                        <p class="text-lg font-semibold {{ $customer->net_profit >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            {{ number_format($customer->net_profit, 0, ',', '.') }} VNĐ
                        </p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">Lãi/Lỗ hôm nay</p>
                        <p class="text-lg font-semibold {{ $customer->daily_net_profit >= 0 ? 'text-green-600' : 'text-red-600' }}">
            ...
            <div class="mb-8">
                ...
                <div class="space-y-4">
                    ...
                    @php
                        $existingRate = $customer->bettingRates->where('betting_type_id', $bettingType->id)->first();
                    @endphp
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" 
                                       ...
                                       value="{{ old('betting_rates.'.$index.'.win_rate', $existingRate ? $existingRate->win_rate : '0.8') }}"
                                       min="0" 
                                       max="1" 
                                       step="0.01"
                            ...
                            <div>
                                ...
                                <input type="number" 
                                       ...
                                       value="{{ old('betting_rates.'.$index.'.lose_rate', $existingRate ? $existingRate->lose_rate : '0.2') }}"
                                       min="0" 
                                       max="1" 
                                       step="0.01"
