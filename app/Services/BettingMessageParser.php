...
class BettingMessageParser
{
    ...
    private $stationAbbreviations = [
        ...
        // Thêm station name đầy đủ
        'tây ninh' => 'tây ninh','tayninh' => 'tây ninh','tay ninh' => 'tây ninh',
        'an giang' => 'an giang','angiang' => 'an giang','an giang' => 'an giang',
        'bến tre' => 'bến tre','bentre' => 'bến tre','ben tre' => 'bến tre',
        'tp.hcm' => 'tp.hcm','tphcm' => 'tp.hcm','tp hcm' => 'tp.hcm',
        'đồng nai' => 'đồng nai','dongnai' => 'đồng nai','dong nai' => 'đồng nai',
        'hà nội' => 'hà nội','hanoi' => 'hà nội','ha noi' => 'hà nội',
        'vũng tàu' => 'vũng tàu','vungtau' => 'vũng tàu','vung tau' => 'vũng tàu',
        'bình dương' => 'bình dương','binhduong' => 'bình dương','binh duong' => 'bình dương',
        'cần thơ' => 'cần thơ','cantho' => 'cần thơ','can tho' => 'cần thơ',
        'long an' => 'long an','longan' => 'long an','long an' => 'long an',
        'cà mau' => 'cà mau','camau' => 'cà mau','ca mau' => 'cà mau',
        'đồng tháp' => 'đồng tháp','dongthap' => 'đồng tháp','dong thap' => 'đồng tháp',
        'bạc liêu' => 'bạc liêu','baclieu' => 'bạc liêu','bac lieu' => 'bạc liêu',
        'sóc trăng' => 'sóc trăng','soctrang' => 'sóc trăng','soc trang' => 'sóc trăng',
        'bình phước' => 'bình phước','binhphuoc' => 'bình phước','binh phuoc' => 'bình phước',
        'bình thuận' => 'bình thuận','binhthuan' => 'bình thuận','binh thuan' => 'bình thuận',
        'kiên giang' => 'kiên giang','kiengiang' => 'kiên giang','kien giang' => 'kiên giang',
        'hậu giang' => 'hậu giang','haugiang' => 'hậu giang','hau giang' => 'hậu giang',
        'tiền giang' => 'tiền giang','tiengiang' => 'tiền giang','tien giang' => 'tiền giang',
        'đà lạt' => 'đà lạt','dalat' => 'đà lạt','da lat' => 'đà lạt',
        'vĩnh long' => 'vĩnh long','vinhlong' => 'vĩnh long','vinh long' => 'vĩnh long',
        'trà vinh' => 'trà vinh','travinh' => 'trà vinh','tra vinh' => 'trà vinh',
    ];
    ...
    private function parseLine(string $line): array
    {
        ...
        $dToggle        = 0;
        $results        = [];
        $hasStationInMessage = false;
        $lastBettingType = null;
        $multipleBets = [];
        $regionFromMessage = $this->parseRegionFromMessage($line);

        \Log::info('=== FLUSH START ===', [
            'currentNumbers' => $currentNumbers,
            'pending_count' => count($pending),
            'lastSelectedStations' => $this->lastSelectedStations
        ]);
        ...
        $flush = function() use (&$results,&$currentStation,&$stationBuffer,&$currentNumbers,&$pending,&$dToggle,&$hasStationInMessage,&$regionFromMessage) {
            ...
            foreach ($pending as $dir) {
                ...
                if ($dir['type'] === '__MULTI_STATIONS__') {
                    ...
                    // KHÔNG set lại lastSelectedStations ở đây
                    continue;
                }
                
                // Xác định danh sách đài cho directive này
                $dirStations = [];
                if (!empty($dir['station'])) {
                    $dirStations = is_array($dir['station']) ? $dir['station'] : [$dir['station']];
                } else {
                    if (!empty($this->lastSelectedStations)) {
                        $dirStations = $this->lastSelectedStations;
                    } else {
                        $dirStations = $targets;
                    }
                }
                ...
                if (!empty($dir['xien_size'])) {
                    // Xác định dãy số để tạo xiên
                    $numbersToUse = !empty($dir['numbers']) ? $dir['numbers'] : $expandedNumbers;
                    $groups = $this->combinations($numbersToUse, (int)$dir['xien_size']);
                    
                    foreach ($dirStations as $station) {
                ...
                foreach ($dirStations as $station) {
                    foreach ($expandedNumbers as $num) {
                        $type = ($dir['type'] === 'bao_lo_dynamic') 
                            ? $this->mapLoTypeByLen($num) 
                            : $dir['type'];
            ...
            $stationBuffer = [];
            $dToggle = 0;
            // KHÔNG reset lastSelectedStations
        };
        ...
        for ($i=0; $i<count($tokens); $i++) {
            ...
            // 0) Multi-stations: 2d/3d/4d (+ miền)
            if (preg_match('/^([234])d(ai)?(m?(bac|trung|nam))?$/u', $tk, $m)) {
                $n = (int)$m[1];
                $region = isset($m[4]) ? $m[4] : null;
            
                // RESET lastSelectedStations khi gặp multi-station directive mới
                $this->lastSelectedStations = null;
                ...
                // Parse các đài cụ thể sau 2dai/3dai/4dai
                $specifiedStations = [];
                $j = $i + 1;
                while ($j < count($tokens) && count($specifiedStations) < $n) {
                    $nextToken = $tokens[$j];
                    
                    if ($this->isStationToken($nextToken)) {
                        $station = $this->normalizeStation($nextToken);
                        $specifiedStations[] = $station;
                        $j++;
                    } else {
                        $stationFromTokens = $this->isStationTokens($tokens, $j);
                        if ($stationFromTokens) {
                            $specifiedStations[] = $stationFromTokens;
                            $j += $this->getStationTokenLength($tokens, $j);
                        } else {
                            break;
                        }
                    }
                ...
                if (!empty($specifiedStations)) {
                    // Có đài cụ thể
                    $this->lastSelectedStations = $specifiedStations;
                    $i = $j - 1;
                } else {
                    // KHÔNG có đài cụ thể -> Lấy theo lịch
                    $region = $region ?? $regionFromMessage ?? 'nam';
                    
                    // Lấy đài chính
                    $mainStation = $this->getDefaultStationByDay($region, $this->globalDate);
                    \Log::info('Main station', ['main' => $mainStation]);
                    // Lấy đài phụ theo lịch
                    $subStations = $this->getSubStationsByDay($region, $this->globalDate);
                    \Log::info('Sub stations from schedule', ['sub' => $subStations]);
                    $subStations = array_values(array_diff($subStations, [$mainStation]));
                    // Kết hợp: 1 đài chính + (n-1) đài phụ
                    $selectedStations = [$mainStation];
                    for ($s = 0; $s < min($n - 1, count($subStations)); $s++) {
                        $selectedStations[] = $subStations[$s];
                    }
                    \Log::info('After combining main + sub', ['selected' => $selectedStations, 'needed' => $n]);
                    // Nếu không đủ đài phụ theo lịch, random thêm
                    if (count($selectedStations) < $n) {
                        $availableStations = $this->getStationsByRegion($region);
                        $remainingStations = array_diff($availableStations, $selectedStations);
                        $needed = $n - count($selectedStations);
                        $randomPicked = array_slice(array_values($remainingStations), 0, $needed);
                        $selectedStations = array_merge($selectedStations, $randomPicked);
                    }
            ...
            if ($this->isStationToken($tk)) {
                ...
                // CLEAR lastSelectedStations khi gặp station riêng lẻ (đài mới)
                $this->lastSelectedStations = null;
                
                $stationBuffer = [$station]; // Reset buffer
                $currentStation = $station;
                $hasStationInMessage = true;
                continue;
            }
            
            // 1.b) Kiểm tra các token liên tiếp có thể tạo thành station
            ...
            // 3) Directive (lo/d/xc/da/xien/dd/...)
            // Trong vòng for, phần xử lý directive:
            if ($ret = $this->parseDirectiveAt($tokens, $i, $dToggle)) {
                \Log::info('Parsed directive', [
                    'type' => $ret['directive']['type'],
                    'has_numbers' => !empty($ret['directive']['numbers']),
                    'numbers' => $ret['directive']['numbers'] ?? null,
                    'amount' => $ret['directive']['amount'] ?? 0,
                    'advance' => $ret['advance']
                ]);
                ...
                if (!empty($ret['attach_number'])) {
                    $flush();
                    $currentNumbers = [$ret['attach_number']];
                }
                
                // Nếu directive có numbers (xien với số đi kèm)
                if (!empty($ret['directive']['numbers'])) {
                    \Log::info('Directive has numbers - will flush immediately');
                    $flush(); // Flush pending trước
                    $currentNumbers = $ret['directive']['numbers'];
                    
                    // Xác định đài cho directive
                    $currentStationForDirective = null;
                    if (!empty($this->lastSelectedStations)) {
                        $currentStationForDirective = array_values(array_unique($this->lastSelectedStations)); // UNIQUE
                    } elseif (!empty($stationBuffer)) {
                    ...
                    \Log::info('Station for directive with numbers', [
                        'station' => $currentStationForDirective
                    ]);
                    
                    // Thêm directive vào pending
                    ...
                    if (is_array($currentStationForDirective)) {
                        foreach ($currentStationForDirective as $station) {
                            ...
                            \Log::info('Added directive to pending', ['station' => $station, 'pending_count' => count($pending)]);

                        }
                    } else {
                        $directive['station'] = $currentStationForDirective;
                        $pending[] = $directive;
                        \Log::info('Added directive to pending', ['station' => $currentStationForDirective, 'pending_count' => count($pending)]);

                    }
                    
                    // FLUSH NGAY để tạo bet với số hiện tại
                    \Log::info('Flushing immediately for directive with numbers');

                    $flush();
                    
                    // Reset để chuẩn bị cho directive tiếp theo
                    $currentNumbers = [];
                ...
                // Logic cũ cho các directive không có numbers
                $currentStationForDirective = null;
                if (!empty($this->lastSelectedStations)) {
            ...
            // 4) Số (2~4) hoặc từ-khóa-dãn
            if ($this->isNumberToken($tk) || $this->isNumberKeyword($tk)) {
                $next = $tokens[$i+1] ?? null;
                
                // Thu thập dãy số liên tiếp
                $numbersSequence = [];
                ...
                // Kiểm tra xem token sau dãy số có phải là amount không
                $tokenAfterNumbers = $tokens[$j] ?? null;
                
                // Implicit betting: dãy số + amount (dùng lại directive trước)
                if ($tokenAfterNumbers && ($amount = $this->parseAmountOnly($tokenAfterNumbers)) > 0) {
                    if (!empty($lastBettingType)) {
                        ...
                        $stationsToUse = [];
                        if (!empty($this->lastSelectedStations)) {
                            $stationsToUse = $this->lastSelectedStations;
                        } elseif (!empty($stationBuffer)) {
                            $stationsToUse = [end($stationBuffer)];
                        } elseif ($currentStation) {
                            $stationsToUse = [$currentStation];
                        } else {
                            $stationsToUse = [$this->getDefaultStationByDay($regionFromMessage, $this->globalDate)];
                        }
                        
                        // Nếu là xiên, tạo tổ hợp
                        if ($lastBettingType === 'da_xien') {
                            $xienSize = 2; // Default
                            $groups = $this->combinations($numbersSequence, $xienSize);
                            
                            foreach ($stationsToUse as $station) {
                                foreach ($groups as $grp) {
                                    $multipleBets[] = [
                                        'station' => $station,
                                        'type' => 'da_xien',
                                        'numbers' => $grp,
                                        'amount' => $amount,
                                        'meta' => ['xien_size' => $xienSize]
                                    ];
                                }
                            }
                        } else {
                            // Loại cược khác
                            $betType = ($lastBettingType === 'bao_lo_dynamic') 
                                ? $this->mapLoTypeByLen($numbersSequence[0]) 
                                : $lastBettingType;
                        ...
                        $i = $j; // Skip đến sau amount
                        continue;
                    }
                }
            
                // Không phải implicit betting - thu thập số bình thường
                $currentNumbers = $numbersSequence;
                $i = $j - 1;
                continue;
            }
        }
        ...
        $flush();
        return array_merge($results, $multipleBets);
    }
    ...
    private function parseDirectiveAt(array $tokens, int $i, int $dToggle): ?array
    {
        ...
        // 1.a) Dạng dính: d35n / d40n / d130k / d0k
        ...
        // ===== 2) LÔ / BAO LÔ =====
        {
            ...
            if (($attached && $attached['base'] !== 'xc' && $attached['base'] !== 'd') || $this->isTypeToken($tk, ['lo','bao_lo','bao3_lo','bao4_lo','bd','baodao','bdao','bld','bldao','daolo'])) {
                ...
                if ($amount >= 0) {
                    $type = $this->aliasToType($typeToken);
        ...
        if ($this->isTypeToken($tk, ['xien','dx','xien2','xien3','xien4','xhai','xba','xbon','dax','đax'])) {
            ...
            // Thu thập TẤT CẢ các số sau xien cho đến khi gặp amount
            $numbersForXien = [];
            $amountToken = null;
            $amountIndex = -1;
            ...
            for ($j = $i + 1; $j < count($tokens); $j++) {
                ...
                // Nếu gặp directive khác hoặc station, dừng
                if ($this->isTypeToken($token, array_keys($this->bettingShortcuts)) || 
                    $this->isStationToken($token)) {
                    break;
                }
                
                // Nếu là số, thu thập
                if ($this->isNumberToken($token)) {
                    $numbersForXien[] = $token;
                    continue; // Tiếp tục thu thập số
                }
                
                // Nếu là amount, dừng
                if (($v = $this->parseAmountOnly($token)) !== false && $v > 0) {
                    $amountToken = $token;
                    $amountIndex = $j;
                    break;
                }
            ...
            \Log::info('Collected xien numbers', [
                'numbers' => $numbersForXien,
                'amount' => $amountToken,
                'amountIndex' => $amountIndex
            ]);
            
            // Nếu có số và amount
            if (!empty($numbersForXien) && $amountToken && ($v = $this->parseAmountOnly($amountToken)) > 0) {
                return [
                    'directive' => [
                        'type' => 'da_xien',
                        'amount' => $v,
            ...
            // Fallback: xien <amount> (áp cho số đã có trước đó)
            if ($next && ($v = $this->parseAmountOnly($next)) > 0) {
                return [
                    'directive' => ['type'=>'da_xien', 'amount'=>$v, 'xien_size'=>$size], 
                    'advance' => 1, 
                    'dToggle' => $dToggle
                ];
            }
            ...
            if ($next && preg_match('/^(\d+(?:\.\d+)?)n$/', $next, $m)) {
                $amount = $this->toVnd($m[1]);
                return [
                    'directive' => ['type'=>'da_xien', 'amount'=>$amount, 'xien_size'=>$size], 
                    'advance' => 1, 
                    'dToggle' => $dToggle
                ];
            }
        ...
        if (preg_match('/^(\d+)dai$/u', $tk, $m)) {
            ...
            $meta = ['multi_stations' => $n, 'region' => null];
            if (!empty($specifiedStations)) {
                $meta['specified_stations'] = $specifiedStations;
                return ['directive'=>['type'=>'__MULTI_STATIONS__','amount'=>0,'meta'=>$meta], 'advance'=>$j - $i - 1, 'dToggle'=>$dToggle];
            }
        
            // KHÔNG có đài cụ thể -> sẽ lấy theo lịch ở parseLine
            return ['directive'=>['type'=>'__MULTI_STATIONS__','amount'=>0,'meta'=>$meta], 'advance'=>0, 'dToggle'=>$dToggle];
        }
    ...
    private function normalize(string $s): string
    {
        ...
        // 1.1) Xử lý số có dấu chấm nhiều lần trước: 72.79.54 -> 72 79 54
        $s = preg_replace('/(\d+)\.(\d+)\.(\d+)/', '$1 $2 $3', $s);
        
        // 1.2) Xử lý số có dấu chấm: 56.65 -> 56 65 (tách thành 2 số)
    ...
    private function getSubStationsByDay(string $region, ?string $globalDate = null): array
    {
        ...
        $subStationsSchedule = [
            ...
            'friday' => [
                'nam' => ["Vĩnh Long", "Trà Vinh"],
                'trung' => ['Nghệ An', 'Hà Tĩnh'],
                'bac' => []
            ],
            'saturday' => [
                'nam' => ["Long An", "Bình Phước", "Hậu Giang"],
                'trung' => ['Quảng Ngãi', 'Phú Yên'],
                'bac' => []
            ],
            'sunday' => [
                'nam' => ["Kiên Giang", "Đà Lạt"],
    ...
    {
        // Xử lý dấu phẩy trước khi check
        $tk = str_replace(',', ' ', $tk);
    ...
    private function isStationTokens(array $tokens, int $startIndex, int $maxLength = 3): ?string
    {
        for ($len = 1; $len <= $maxLength && $startIndex + $len <= count($tokens); $len++) {
            ...
            // Thử với các biến thể khác nhau
            $variants = [
                $combined, // Gốc
                str_replace(' ', '', $combined), // Không dấu cách
                $this->stripVN($combined), // Không dấu
                $this->stripVN(str_replace(' ', '', $combined)), // Không dấu và không dấu cách
            ];
            
            foreach ($variants as $variant) {
                $normVariant = $this->norm($variant);
                if (isset($this->stationAbbreviations[$normVariant])) {
                    return $this->stationAbbreviations[$normVariant];
                }
            }
        }
    ...
    private function hydrateStationAliasesFromDB(): void
    {
        try {
            ...
            foreach ($stations as $st) {
                ...
                // Thêm station name đầy đủ vào abbreviations
                $this->stationAbbreviations[$normName] = $name;
                $this->stationAbbreviations[str_replace(' ','',$normName)] = $name;
                
                // Thêm các biến thể của tên đài
                $this->stationAbbreviations[$name] = $name; // Tên gốc
                $this->stationAbbreviations[str_replace(' ','',$name)] = $name; // Tên không dấu cách
