...
if ($result['is_valid']) {
    ...
    foreach ($result['multiple_bets'] as $i => $bet) {
        echo ($i+1) . ". Station: {$bet['station']}, Type: {$bet['type']}, Numbers: " . implode(',', $bet['numbers']) . ", Amount: {$bet['amount']}\n";
    }
} else {
...
echo "\n=== EXPECTED OUTPUT ===\n";
echo "Should create:\n";
echo "- 3 bets đá xiên 56 65 for tp.hcm, long an, bình phước\n";
echo "- 3 bets đá xiên 56 59 for tp.hcm, long an, bình phước\n";
echo "- 9 bets đá xiên 72 79 54 for tp.hcm, long an, bình phước\n";
echo "Total: 15 bets\n";
